              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox() : Text(controller.user.value!.displayName??'',
                    style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: onSurfaceTextColor),)),
                    const Spacer(flex: 1,),
                    _DrawerButton(icon: Icons.web, label: 'website',
                      onPressed: () => controller.website(),),
                    _DrawerButton(icon: Icons.facebook, label: 'facebook',
                      onPressed: () => controller.facebook(),),
                    Padding(
                      padding: const EdgeInsets.only(left: 25),
                      child: _DrawerButton(icon: Icons.email, label: 'email',
                        onPressed: () => controller.email,),
                    ),
                    const Spacer(flex: 4,),
                    _DrawerButton(icon: Icons.logout, label: 'logout',
...
class _DrawerButton extends StatelessWidget {
  const _DrawerButton({Key? key, required this.icon,
    required this.label, this.onPressed}) : super(key: key);
